...
public class IA : IA_Utils
    // Gère tous les autres scripts IA. TODO : Faire en sorte que ce script génère tous les autres scripts et leur attribue leurs paramètres, sauf celui de l'arme à feu.
{
    [Header("IA Script")]
    ...
    [Header("IA Health Variable")]
    [SerializeField] private int max_Healf = 100;
    [Header("IA Equipment Variable")]
    ...
    // FixedUpdate est appelé à chaque frame physique
    void FixedUpdate()
    {
        if (ia_Gun == null) // Si au démarrage c'est trop rapide
        {
            ia_Gun = weapon_Empty.GetComponentInChildren<Gun>(); // Trop tôt
            anim.SetBool(ia_Gun.GetComponent<Gun>().Gun_Scriptable.gun_Type, true);
            enemy_Animation_Ragdoll.AddComponent<EnemyIK>(); // Parce que nous n'avons pas encore l'arme
    ...
    {
        spawner = sp;
        // TODO : À faire plus tard quand plus d'armes seront disponibles
    ...
    public void Die()
    {
        ...
        if(spawner != null)
        {
            spawner.Enemy_Die(gameObject); // Préviens le spawner de la mort
        }
        anim.enabled = false;     
        ia_Vie.enabled = false;
        ia_Equipement.enabled = false;
        ia_Gun.Drop_Gun(false, true);
        gameObject.AddComponent<Kill>(); // TODO : À changer plus tard pour laisser les cadavres
